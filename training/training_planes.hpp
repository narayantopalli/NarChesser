...
#include "../src/manipulators.hpp"
#include <vector>
#include <array>
#include <memory> 

namespace planes {
    std::unique_ptr<Bitboard[]> toPlane(const Board& board, Color color);
    std::unique_ptr<Bitboard[]> extraPlanes(const Board& board);
    std::unique_ptr<Bitboard[]> emptyPlane();
}
